...
# from config_pipeline_iterative import AnalysisConfig # Original import
...
def main():
    parser = argparse.ArgumentParser(description="Run the molecular dynamics analysis pipeline with iterative binding metrics and plotting.")
    # Use the iterative config file by default from the configs directory
    parser.add_argument("--config", default="configs/config_pipeline_iterative.py", help="Path to the configuration file.") # Updated default config path
    ...
    try:
        # Import the AnalysisConfig class from the dynamically loaded module
    ...
    steps = [
        ...
        },
        {
            ...
            "command": [
                ...
                "--min-res-time", str(config.residence_event_analysis_min_res_time), # Pass min_res_time from residence analysis config
                "--max-off-time", str(config.residence_event_analysis_max_off_time),
                "--output-dir", config.residence_events_output_dir
            ]
        },
        {
            "name": "Iterative Binding Metrics",
            "script": "iterative_binding_metrics.py",
            "done_file": "iterative_binding_metrics.done", # Updated done file
            "command": [
                sys.executable, os.path.join("src", "iterative_binding_metrics.py"),
                "--events-csv", config.residence_events_csv,
                "--classification-csv", config.pt_classification_csv, # Use pt_classification_csv
                "--properties-csv", config.sim_info_csv, # Use sim_info_csv
                "--sim-info-json", config.sim_info_json, # Use sim_info_json
                "--min-res-time", str(config.residence_event_analysis_min_res_time), # Pass min_res_time from residence analysis config
                "--output-csv", config.iterative_binding_metrics_csv, # Use the new output path
                "--frame-step", str(config.iterative_binding_metrics_frame_step), # Pass frame_step from config
            ],
            "flag_args": [
                ("verbose", config.iterative_binding_metrics_verbose), # Pass verbose flag from config
            ]
        },
        {
            "name": "Plot Facet Metrics",
            "script": "plot_facets.py",
            "done_file": "plot_facets.done", # New done file
            "command": [
                sys.executable, os.path.join("src", "plot_facets.py"),
                config.iterative_binding_metrics_csv, # Input CSV from iterative binding metrics
                "--outdir", config.iterative_binding_metrics_output_dir # Output to the same directory
            ]
        }
    ]
